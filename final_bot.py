        ...
        with st.spinner("جاري جلب البيانات..."):
            try:
                ...
                if not df.empty:
                    ...
                    # وقف خسارة: أقرب دعم لو معقول (بين 0.5×ATR و8% تحت السعر
                    # - مش قريب أوي عشان مايتكسرش بأول تذبذب عادي، ومش بعيد أوي
                    # برضو)، وإلا وقف مبني على ATR
                    min_stop_distance_t1 = max(0.5 * atr_14_t1, price * 0.01)  # على الأقل 1% أو نص ATR
                    support_distance_t1 = (price - nearest_support_t1) if nearest_support_t1 is not None else 0
                    if (nearest_support_t1 is not None
                            and min_stop_distance_t1 <= support_distance_t1 <= price * 0.08):
                        stop_loss_price = round(nearest_support_t1, 2)
                        sl_basis_t1 = "أقرب دعم"
                    else:
                        stop_loss_price = round(max(price - 1.5 * atr_14_t1, price * 0.9), 2)
                        sl_basis_t1 = "مبني على ATR (الدعم إما بعيد جداً أو قريب جداً من السعر)"
                    ...
                    # ===================================================================
                    # Explainable Decision Card - EAGLE INVESTOR OS 2.0 (قسم 31)
                    # ===================================================================
                    st.write("---")
                    st.markdown("## 🦅 EAGLE DECISION CARD")

                    market_regime_t1 = detect_market_regime()
                    avg_trade_value_t1 = price * vol_ma10

                    eagle_components_t1 = {
                        "trend": _score_trend(ema9, ema21, adx_val, price),
                        "momentum": _score_momentum(rsi, mfi, upper, lower, price),
                        "volume_rvol": _score_volume_rvol(rvol_t1, price > yesterday_close_t1),
                        "liquidity": _score_liquidity(avg_trade_value_t1),
                        "risk_reward": _score_risk_reward(rr_ratio_t1),
                        "market_depth": _score_market_depth(depth),
                        "market_regime": market_regime_score_component(market_regime_t1),
                        "breadth": breadth_score_component(st.session_state.get("last_market_breadth")),
                        "fundamentals": _score_fundamentals(fund_score) if any(v is not None for v in fundamentals.values()) else None,
                        "valuation": _score_valuation(graham["graham_upside_%"]),
                        # دول محتاجين سياق مسح شامل (مقارنة بأسهم تانية) - مش متاحين لسهم لوحده
                        "relative_strength": None,
                        "sector_strength": None,
                    }
                    eagle_result_t1 = compute_eagle_score(eagle_components_t1)
                    dist_to_resistance_pct_t1 = ((nearest_resistance_t1 - price) / price * 100) if nearest_resistance_t1 else None
                    entry_quality_t1 = determine_entry_quality(
                        eagle_result_t1["eagle_score"], dist_to_resistance_pct_t1, rr_ratio_t1, depth,
                    )
                    orc_t1 = compute_opportunity_risk_confidence(
                        eagle_components_t1, eagle_result_t1, rr_ratio_t1, atr_pct, depth,
                    )

                    data_statuses_t1 = {
                        "technical": "LIVE",
                        "fundamentals": "DELAYED" if any(v is not None for v in fundamentals.values()) else "UNAVAILABLE",
                        "market_depth": "LIVE" if depth else ("UNAVAILABLE" if not (ticker_input.endswith(".CA") and get_egxapi_key()) else "STALE"),
                        "market_regime": "LIVE" if market_regime_t1 else "UNAVAILABLE",
                        "breadth": "LIVE" if st.session_state.get("last_market_breadth") else "UNAVAILABLE",
                        "relative_strength": "UNAVAILABLE",
                        "sector_strength": "UNAVAILABLE",
                        "early_move": "DISABLED_NO_PROVIDER",
                        "news_impact": "DISABLED_NO_PROVIDER",
                        "macro": "DISABLED_NO_PROVIDER",
                        "geopolitical": "DISABLED_NO_PROVIDER",
                    }
                    confidence_result_t1 = compute_data_confidence(
                        data_statuses_t1,
                        eagle_coverage_ratio=eagle_result_t1["components_used"] / max(eagle_result_t1["components_total"], 1),
                    )

                    conflict_result_t1 = detect_signal_conflicts(
                        eagle_result_t1["eagle_score"], market_regime_t1, rvol_t1, avg_trade_value_t1,
                        depth, confidence_result_t1["confidence_pct"], rr_ratio_t1, price > yesterday_close_t1,
                        st.session_state.get("last_market_breadth"),
                    )
                    final_decision_t1 = make_final_decision(
                        eagle_result_t1["eagle_score"], conflict_result_t1, confidence_result_t1["confidence_pct"],
                        market_regime_t1, avg_trade_value_t1, rr_ratio_t1,
                    )

                    dc1, dc2, dc3 = st.columns(3)
                    dc1.metric("القرار", final_decision_t1)
                    dc2.metric("Eagle Score", f"{eagle_result_t1['eagle_score']}/100" if eagle_result_t1['eagle_score'] is not None else "-")
                    dc3.metric("Data Confidence", f"{confidence_result_t1['confidence_pct']:.0f}%")

                    with st.expander("📋 تفاصيل مكوّنات القرار (Why / Why Not)", expanded=True):
                        comp_labels = {
                            "trend": "الاتجاه", "momentum": "الزخم", "volume_rvol": "الحجم النسبي",
                            "liquidity": "السيولة", "risk_reward": "Risk/Reward", "market_depth": "عمق السوق",
                            "market_regime": "حالة السوق العامة", "breadth": "اتساع السوق",
                            "fundamentals": "التحليل المالي", "valuation": "التقييم (جراهام)",
                            "relative_strength": "القوة النسبية مقابل EGX30", "sector_strength": "قوة القطاع",
                        }
                        for key, weight in EAGLE_WEIGHTS.items():
                            val = eagle_components_t1.get(key)
                            status_note = ""
                            if val is None:
                                status_note = " _(غير متاح دلوقتي - اتشال من الحساب بوزنه، مش صفر)_"
                            st.write(f"- **{comp_labels.get(key, key)}** (وزن {weight}): {val if val is not None else 'N/A'}{status_note}")

                        st.markdown("**Catalysts (أهم نقاط القوة):**")
                        catalysts = []
                        if eagle_components_t1.get("trend", 0) and eagle_components_t1["trend"] >= 10:
                            catalysts.append("اتجاه فني قوي (EMA9 فوق EMA21 + ADX مرتفع)")
                        if rvol_t1 >= 1.5 and price > yesterday_close_t1:
                            catalysts.append(f"حجم تداول قوي نسبياً ({rvol_t1:.1f}x) مترافق مع صعود السعر")
                        if rr_ratio_t1 is not None and rr_ratio_t1 >= 2:
                            catalysts.append(f"نسبة Risk/Reward ممتازة ({rr_ratio_t1})")
                        st.write("\n".join(f"- {c}" for c in catalysts) if catalysts else "- مفيش نقاط قوة واضحة بارزة دلوقتي")

                        st.markdown("**Risks / Conflicts (أهم المخاطر والتضاربات):**")
                        if conflict_result_t1["conflicts"]:
                            for c in conflict_result_t1["conflicts"]:
                                st.write(f"- ⚠️ {c}")
                        else:
                            st.write("- مفيش تضاربات واضحة بين الإشارات دلوقتي")

                        st.markdown("**Invalidation (متى الفرضية دي تبطل):**")
                        st.write(f"- لو السعر كسر وقف الخسارة المقترح ({stop_loss_price} {price_currency})")

                        st.caption(
                            "⚠️ الكارت ده أداة دعم قرار مش ضمان ربح - Technical/Volume/Depth/Regime بس "
        ...
        with st.spinner(f"جاري مسح {len(scan_stocks)} سهم على دفعات ({BATCH_SIZE} سهم لكل دفعة) + إعادة محاولة الأسهم اللي تفشل..."):
            ...
            for idx, (name, ticker) in enumerate(scan_stocks.items()):
                ...
                try:
                    ...
                    # وقف خسارة: أقرب دعم لو معقول (بين 0.5×ATR و8% تحت السعر -
                    # مش قريب أوي عشان مايتكسرش بأول تذبذب عادي)، وإلا ATR
                    min_stop_distance = max(0.5 * atr_14_val, p * 0.01)  # على الأقل 1% أو نص ATR
            ...
            if eagle_raw:
                ...
                for ticker, info in eagle_raw.items():
                    ...
                    comps["breadth"] = breadth_score_component(st.session_state.get("last_market_breadth"))
                    ...
                    # --- Data Confidence + Signal Conflict + Final Decision (Phase 1 Audit fix) ---
                    # دول كانوا متطبقين في تاب الفحص التفصيلي بس - ده بيوحّد
                    # نفس منطق القرار المنضبط في المسح الشامل كمان
                    data_statuses = {
                        "technical": "LIVE",
                        "fundamentals": "DELAYED" if info["fundamentals_available"] else "UNAVAILABLE",
                        "market_depth": "LIVE" if info["depth_metrics"] else "UNAVAILABLE",
                        "market_regime": "LIVE" if market_regime_info else "UNAVAILABLE",
                        "breadth": "LIVE",  # بيتحسب من نفس المسح الجاري، متاح دايماً هنا
                        "early_move": "DISABLED_NO_PROVIDER",
                        "news_impact": "DISABLED_NO_PROVIDER",
                        "macro": "DISABLED_NO_PROVIDER",
                        "geopolitical": "DISABLED_NO_PROVIDER",
                    }
                    confidence_result = compute_data_confidence(
                        data_statuses,
                        eagle_coverage_ratio=eagle_result["components_used"] / max(eagle_result["components_total"], 1),
                    )
                    conflict_result = detect_signal_conflicts(
                        eagle_result["eagle_score"], market_regime_info, info["rvol"], info["avg_trade_value"],
                        info["depth_metrics"], confidence_result["confidence_pct"], info["rr_ratio"],
                        info["price_up_today"], st.session_state.get("last_market_breadth"),
                    )
                    final_decision = make_final_decision(
                        eagle_result["eagle_score"], conflict_result, confidence_result["confidence_pct"],
                        market_regime_info, info["avg_trade_value"], info["rr_ratio"],
                    )
                    ...
                    info["data_entry"]["Data Confidence %"] = confidence_result["confidence_pct"]
                    info["data_entry"]["القرار النهائي"] = final_decision
                    if "BUY CANDIDATE" in final_decision:
                        rank = 0
                    elif "WAIT" in final_decision:
                        rank = 1
                    elif "NO TRADE" in final_decision:
                        rank = 2
                    else:  # AVOID
                        rank = 3
                    info["data_entry"]["ترتيب_القرار"] = rank
                    info["data_entry"]["تضاربات الإشارات"] = "؛ ".join(conflict_result["conflicts"]) if conflict_result["conflicts"] else "لا يوجد"
            ...
            if all_results:
                ...
                has_new_decision = "ترتيب_القرار" in all_df.columns
                has_old_verdict = "ترتيب_التوصية" in all_df.columns

                if has_new_decision:
                    verdict_cols = [c for c in [
                        "اسم الشركة", "الرمز البرمجي", "القطاع", "العملة", "السعر الحالي",
                        "القرار النهائي", "🦅 Eagle Score (100)", "Data Confidence %",
                        "التوصية", "النقاط الفنية والسيولة (من 100)", "الدرجة المالية (من 100)",
                    ] if c in all_df.columns]
                    sort_cols = [c for c in ["ترتيب_القرار", "🦅 Eagle Score (100)"] if c in all_df.columns]
                    st.dataframe(
                        all_df.sort_values(by=sort_cols, ascending=[True] + [False] * (len(sort_cols) - 1))[verdict_cols],
                        use_container_width=True,
                    )
                    st.caption(
                        "**القرار النهائي** هو المرجع الأساسي (بيراعي تضارب الإشارات وجودة البيانات مش بس الدرجة الفنية/المالية). "
                        "عمود **التوصية** القديم لسه ظاهر جنبه كتفصيل تانوي بس - ممكن يختلف عن القرار النهائي في حالات فيها تضارب إشارات."
                    )
                    dcounts = all_df["القرار النهائي"].value_counts()
                    dc1, dc2, dc3, dc4 = st.columns(4)
                    dc1.metric("🟢 BUY CANDIDATE", int(sum(v for k, v in dcounts.items() if "BUY CANDIDATE" in k)))
                    dc2.metric("🟡 WAIT", int(sum(v for k, v in dcounts.items() if "WAIT" in k)))
                    dc3.metric("🔴 AVOID", int(sum(v for k, v in dcounts.items() if "AVOID" in k)))
                    dc4.metric("⚪ NO TRADE", int(sum(v for k, v in dcounts.items() if "NO TRADE" in k)))

                    if has_old_verdict:
                        with st.expander("📋 عرض التوصية القديمة (فني+مالي بس، من غير فحص تضارب أو جودة بيانات)"):
                ...
                elif has_old_verdict:
                    # Fallback لو لأي سبب القرار الجديد مش متاح للنتايج دي
                    verdict_cols = [c for c in [
                        "اسم الشركة", "الرمز البرمجي", "القطاع", "العملة", "السعر الحالي",
                        "التوصية", "النقاط الفنية والسيولة (من 100)", "الدرجة المالية (من 100)",
                    ] if c in all_df.columns]
