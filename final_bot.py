...
# --- قاموس القيم العادلة للأسهم (تحديث دوري) ---
FAIR_VALUES = {
    "البنك التجاري الدولي": 92.77,
    "السويدي إليكتريك": 38.27,
    "مصرف أبوظبي الإسلامي": 55.62,
    "مجموعة طلعت مصطفى": 23.81
    # أضف هنا باقي الشركات وقيمها العادلة
}

def get_fair_value(name):
    """جلب القيمة العادلة من القاموس"""
    return FAIR_VALUES.get(name, None)
        ...
        with st.spinner("جاري مسح الأسهم ورصد الفرص الصارمة..."):
            ...
            for idx, (name, ticker) in enumerate(ALL_EGX_STOCKS.items()):
                ...
                try:
                    ...
                    # --- التعديل هنا: دمج القيمة العادلة مع الاحتفاظ بالباقي ---
                    fv = get_fair_value(name)
                    ...
                    data_entry = {
                        ...
                        "القيمة العادلة": fv if fv else "N/A",
